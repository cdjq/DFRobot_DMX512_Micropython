...
class DFRobot_RTU(object):
  ...
  def __init__(self, baud, dePin = -1, waitTime = 10):
    '''
      @brief Serial initialization.
      @param baud:  The UART baudrate of esp32s3
      @param waitTime: send data wait time
    '''
    self._ser = UART(1, baudrate=baud, tx = Pin(12), rx = Pin(13))
    self._timeout = 0.1 #0.1s
    self.dePin = dePin
    self._waitTime = waitTime
    self._pin = Pin(dePin, Pin.OUT)
  ...
  def _send_package(self, l):
    ...
    if self.dePin > 0:
      self._pin.value(1)
    if len(l):
      self._ser.write(bytearray(l))
      time.sleep_ms(self._waitTime)
    if self.dePin > 0:
      self._pin.value(0)
